                                  ...
                                  Point *intersection)
{
    ...
    //ЕУМЙ ИПФШ ПДЙО ПФТЕЪПЛ ЧЩТПЦДЕО Ч ФПЮЛХ - ЧЩИПДЙН
    ...
    if(fabs(m1) > fabs(n1))
    {
        ...
        if(denominator == 0)    //РТСНЩЕ РБТБММЕМШОЩ
        {
            return false;
        }
...
bool GeometryHelper::contains(const std::vector<Point> &polygon, const Point &point)
{
    ...
    for(std::vector<Point>::const_iterator it = polygon.begin(); it != end; ++it)
    {
        ...
        else
        {
            ...
            {
                if(((it - 1)->y - currentIntersection.y) * ((it + 1)->y - currentIntersection.y) < 0) //ОЕ ЧЩУФХР
                {
                    --intersectionCount;
                }
            }
    ...
    if(GeometryHelper::intersection(make_pair(*(polygon.end() - 1), *polygon.begin() ), make_pair(point, rightPoint), LineTypeSegment, LineTypeRay, false, &currentIntersection) )
    {
        ...
        {
            if((polygon[polygon.size() - 1].y - firstIntersection.y) * (polygon[0].y - firstIntersection.y) < 0) //ОЕ ЧЩУФХР
            {
                --intersectionCount;
            }
        }
...
double GeometryHelper::distance(const Point point, const pair<Point, Point> segment)
{
    Point projected;
